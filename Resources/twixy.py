...
# customize at 2009 Jan 21  by mizchi 
# Use GAE Proxy like a Birdnest ,that go through GAE OAuth Whitelisti
# About birdnest, check here - Project Hosting on Google Code - http://code.google.com/p/birdnest/
#
#How to use 
#  import twixy
#  api=twixy.Api(
...
class Api(object):
  ...
  def GetHomeTimeline(self, user=None, count=None, since=None, since_id=None):
    ...
    if since_id:
      parameters['since_id'] = since_id
    if user:
      url = self.proxy+'/statuses/home_timeline/%s.json' % user
    elif not user and not self._username:
      raise TwitterError("User must be specified if API is not authenticated.")
    else:
      url = self.proxy+'/statuses/home_timeline.json'
...
#    if not isinstance(status, unicode):
 #     stauts = unicode(status, self._input_encoding) 

    if len(status) > CHARACTER_LIMIT:
      raise TwitterError("Text must be less than or equal to %d characters. "
                         "Consider using PostUpdates." % CHARACTER_LIMIT)

    data = {'status': status.decode('utf-8')}
